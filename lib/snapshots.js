...
const createStore = ({ snapshotPaths, dataPaths } = {}) => {
  ...
  return {
    ...
    async generate(name, generator, force = false) {
    ...
    async generateAll(generator, force = false) {
      ...
      return Promise.all(
        [...$store.keys()].map((name) => this.generate(name, generator, force))
      );
    }
  }
};
